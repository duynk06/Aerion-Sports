...
import com.example.datn1.service.MauSacService;
...
public class MauSacController {
    ...
    public ResponseEntity<?> search(
            ...
            @RequestParam(value = "keyword", required = false) String keyword // Thêm đón keyword từ Vue
    ) {
        Page<MauSacResponse> result = service.search(page, size, trangThai, keyword);
        return ResponseEntity.ok(result);
    }

    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody MauSacRequest r) {
        return ResponseEntity.ok(service.save(r));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody MauSacRequest r) {
