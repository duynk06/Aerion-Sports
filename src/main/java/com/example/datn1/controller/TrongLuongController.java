...
import com.example.datn1.service.TrongLuongService;
...
public class TrongLuongController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<TrongLuongResponse>> getAll() {
    ...
    public ResponseEntity<?> search(
        ...
        Page<TrongLuongResponse> result = service.search(page, size, trangThai, keyword);
        return ResponseEntity.ok(result);
    }

    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody TrongLuongRequest r) {
        return ResponseEntity.ok(service.save(r));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody TrongLuongRequest r) {
