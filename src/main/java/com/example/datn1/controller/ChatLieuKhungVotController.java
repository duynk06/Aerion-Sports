...
import com.example.datn1.service.ChatLieuKhungVotService;
...
@RestController
@RequestMapping("/api/chat-lieu-khung-vot")
@CrossOrigin("*")
public class ChatLieuKhungVotController {

    @Autowired
    private ChatLieuKhungVotService service;

    @GetMapping("/all")
    public ResponseEntity<List<ChatLieuKhungVotResponse>> getAll() {
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody ChatLieuKhungVotRequest r) {
        return ResponseEntity.ok(service.save(r));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody ChatLieuKhungVotRequest r) {
