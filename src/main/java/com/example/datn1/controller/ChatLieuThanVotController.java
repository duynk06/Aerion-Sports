...
import com.example.datn1.service.ChatLieuThanVotService;
...
@RestController
@RequestMapping("/api/chat-lieu-than-vot")
@CrossOrigin("*")
public class ChatLieuThanVotController {

    @Autowired
    private ChatLieuThanVotService service;
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody ChatLieuThanVotRequest r) {
        return ResponseEntity.ok(service.save(r));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody ChatLieuThanVotRequest r) {
