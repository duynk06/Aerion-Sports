...
import com.example.datn1.service.XuatXuService;
...
public class XuatXuController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<XuatXuResponse>> getAll() {
    ...
    public ResponseEntity<?> search(
        ...
        Page<XuatXuResponse> result = service.search(page, size, trangThai, keyword);
        return ResponseEntity.ok(result);
    }

    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody XuatXuRequest r) {
        return ResponseEntity.ok(service.save(r));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody XuatXuRequest r) {
