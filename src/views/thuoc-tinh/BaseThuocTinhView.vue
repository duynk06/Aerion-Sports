...
const loadData = async () => {
  try {
    const res = await myAxios.get(`/api/${props.apiPath}/search`, {
...
const submitThemMoi = async () => {
  try {
    ...
    await myAxios.post(`/api/${props.apiPath}`, payload);
...
const submitCapNhatTen = async () => {
  try {
    ...
    await myAxios.put(`/api/${props.apiPath}/${editingItemId.value}`, payload);
...
const toggleTrangThaiNhanh = async (item) => {
  ...
  try {
    await myAxios.put(`/api/${props.apiPath}/${item.id}/trang-thai`, null, {
    ...
    try {
      item.trangThai = trangThaiMoi;
      await myAxios.put(`/api/${props.apiPath}/${item.id}`, item);
    } catch (err) {
