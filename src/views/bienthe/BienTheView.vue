        ...
        <table class="custom-data-table">
          <thead>
            <tr>
              ...
              <th style="width: 90px; text-align: center;">% Giảm</th>
          ...
          <tbody>
            <tr v-for="(bt, index) in danhSachBienThe" :key="bt.id">
              ...
              <td style="text-align: center; vertical-align: middle;">
                <div class="variant-image-wrapper">
                  <img 
                    :src="bt.hinhAnh ? 'http://localhost:8080' + bt.hinhAnh : 'https://placehold.co/50x50?text=No+Image'" 
                    alt="Ảnh" 
              ...
              <td><span class="badge-prop gray">{{ productInfo.tenDanhMuc }}</span></td>
              <td>{{ productInfo.tenChuViCanVot }}</td>
              <td>{{ productInfo.tenDoCung }}</td>
              <td>{{ productInfo.tenDiemCanBang }}</td>
              <td>{{ productInfo.tenChatLieuThanVot }}</td>
              <td>{{ productInfo.tenChatLieuKhungVot }}</td>
            ...
            <tr v-if="danhSachBienThe.length === 0">
              <td colspan="16" class="empty-table-row">Không tìm thấy chi tiết biến thể nào khớp với bộ lọc!</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="qr-modal-content">
        ...
        <div class="modal-body-layout">
          <div class="info-specs-column">
            ...
            <div class="spec-row-item"><span>Danh mục vợt:</span> <strong>{{ productInfo.tenDanhMuc }}</strong></div>
            <div class="spec-row-item"><span>Chu vi cán:</span> <strong>{{ productInfo.tenChuViCanVot }}</strong></div>
            <div class="spec-row-item"><span>Độ cứng thân:</span> <strong>{{ productInfo.tenDoCung }}</strong></div>
            <div class="spec-row-item"><span>Điểm cân bằng:</span> <strong>{{ productInfo.tenDiemCanBang }}</strong></div>
            <div class="spec-row-item"><span>Chất liệu thân:</span> <strong>{{ productInfo.tenChatLieuThanVot }}</strong></div>
            <div class="spec-row-item"><span>Chất liệu khung:</span> <strong>{{ productInfo.tenChatLieuKhungVot }}</strong></div>
...
// 🌟 ĐÃ SỬA: Khởi tạo biến lưu trữ đầy đủ 6 thông số kỹ thuật nền của sản phẩm cha
const productInfo = ref({ 
  id: route.query.idSP || '', ma: route.query.maSP || '', ten: route.query.tenSP || '',
  tenDoCung: 'Đang tải...', tenDiemCanBang: 'Đang tải...', tenChuViCanVot: 'Đang tải...', 
  tenDanhMuc: 'Đang tải...', tenChatLieuThanVot: 'Đang tải...', tenChatLieuKhungVot: 'Đang tải...'
});
...
const fetchDataBienThe = async () => {
  try {
    ...
    const spCha = listProducts.find(p => Number(p.id) === Number(productInfo.value.id));
    ...
    // 🚀 2. Gọi API lấy danh sách biến thể con được phân trang
    const paramsPayload = {
...
const chuyenSangTrangThemMoi = () => {
  router.push({
    ...
    query: { idSP: productInfo.value.id, maSP: productInfo.value.ma, tenSP: productInfo.value.ten }
  });
};
...
const chuyenSangTrangChinhSua = (bt) => {
  router.push({
    ...
    query: { id: bt.id, idSP: productInfo.value.id, maSP: productInfo.value.ma, tenSP: productInfo.value.ten }
  });
};
...
const exportToExcelConfirm = () => {
  ...
  const dataExport = danhSachBienThe.value.map((item, idx) => ({
    ...
    // Đổ dữ liệu thông số nền cố định lấy từ cha vào file Excel xuất ra
    "Danh mục phân loại": productInfo.value.tenDanhMuc,
    "Chu vi cán": productInfo.value.tenChuViCanVot,
    "Độ cứng thân": productInfo.value.tenDoCung,
    "Điểm cân bằng": productInfo.value.tenDiemCanBang,
    "Chất liệu thân": productInfo.value.tenChatLieuThanVot,
    "Chất liệu khung": productInfo.value.tenChatLieuKhungVot,
...
<style scoped>
/* Giữ nguyên vẹn 100% mã CSS Scoped ban đầu của bạn */
...
.btn-back-to-product { background-color: #fff; border: 1px solid #f79b66; color: #f79b66; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s ease; }
.btn-back-to-product:hover { background-color: #f79b66; color: #fff; }
.box-alert-info { background-color: #fff7ed; border-left: 4px solid #f79b66; padding: 12px 15px; font-size: 14px; margin-bottom: 20px; color: #7c2d12; border-radius: 0 8px 8px 0; text-align: left; }
...
.filter-item { display: flex; flex-direction: column; gap: 8px; text-align: left; flex: 1; min-width: 200px; }
.filter-item label { font-size: 13px; font-weight: 600; color: #4a5568; }
.filter-item input, .filter-item select { padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 13px; outline: none; box-sizing: border-box; width: 100%; height: 38px; transition: border-color 0.2s; }
.filter-item input:focus, .filter-item select:focus { border-color: #f79b66; }
.filter-buttons-group { display: flex; gap: 10px; }
.btn-trigger-filter { background: #e6f7f0; color: #0aa06e; border: none; padding: 10px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; height: 38px; transition: all 0.2s; }
.btn-trigger-filter:hover { background: #ccefe3; }
.btn-reset-filter { background: #f3f4f6; color: #4b5563; border: none; padding: 10px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; height: 38px; transition: all 0.2s; }
...
.table-header-flex { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid #edf2f7; }
.table-summary-title { font-size: 16px; font-weight: 600; color: #1a202c; margin: 0; }
.btn-add-new-orange { background-color: #f79b66; color: #fff; border: none; padding: 9px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s ease; }
.btn-add-new-orange:hover { background-color: #e08553; transform: translateY(-1px); }
...
.custom-data-table th { background-color: #f79b66; color: #fff; padding: 14px 10px; font-size: 14px; font-weight: 600; text-align: center; }
...
.badge-prop.gray { background: #f1f5f9; color: #475569; border: 1px solid #e2e8f0; }
.badge-discount-percent { background-color: #fee2e2; color: #ef4444; padding: 3px 8px; border-radius: 4px; font-weight: 700; font-size: 12px; display: inline-block; }
.text-muted-dash { color: #a0aec0; font-weight: 500; }
.original-price-strike { color: #a0aec0; font-size: 12px; text-decoration: line-through; display: block; margin-bottom: 2px; }
.discounted-price-text { color: #ef4444; font-weight: 700; font-size: 14px; }
.normal-price-text { color: #2563eb; font-weight: 600; font-size: 14px; }
.empty-table-row { text-align: center; padding: 40px !important; color: #a0aec0; font-style: italic; }
...
.toggle-track { position: relative; display: inline-block; width: 34px; height: 18px; border-radius: 999px; transition: background-color 0.2s ease; background-color: #cbd5e1; }
